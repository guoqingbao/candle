...
impl Content {
    pub fn read<R: std::io::Seek + std::io::Read>(reader: &mut R) -> Result<Self> {
        ...
        for _idx in 0..tensor_count {
            ...
            // Keep every GGML IQ format as its original raw dtype. CUDA
            // dispatches native IQ dequant/GEMM kernels; it must not silently
            // requantize these weights while loading the GGUF.
